...
class App(ctk.CTk):
    ...
    def on_closing(self):
        """处理窗口关闭事件，保存未完成的计时记录"""
        if self.timer_frame.is_running:
    ...
    def switch_to_normal_mode(self):
        ...
        self.attributes('-alpha', 0.98)
        self.sidebar_frame.grid(row=0, column=0, sticky="nsew")
        # 【修复2】先停止计时，再显示 TimerFrame，确保 Toast 正确显示
...
class TimerFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self._after_id = None  # 【修复8】跟踪 after 回调 ID
    ...
    def stop_timer(self, show_toast=True):
        ...
        Args:
            show_toast: 是否显示保存成功提示，窗口关闭时设为 False
        """
        if self.is_running:
            self.is_running = False
            # 【修复8】取消挂起的 after 回调
...
class StatisticsFrame(ctk.CTkFrame):
    ...
    def animate_load(self, mode):
        ...
        max_val = 0
        if sorted_users:
            max_val = sorted_users[0][1]['seconds'] / 3600
        ...
        names = [u[0] for u in top_users][::-1]
        self.final_values = [u[1]['seconds'] / 3600 for u in top_users][::-1]
        
        # 记录实际用户数，用于动态调整条形高度
        self.actual_user_count = len(names)
        ...
        # 确保 X 轴有合理的最小范围
        self.chart_max_xlim = max(max_val * 1.15, 0.1) if max_val > 0 else 1
    ...
    def update_chart_animation(self):
        ...
        # 动态调整条形高度：用户越少，条越窄
        user_count = self.actual_user_count
        if user_count <= 1:
            bar_height = 0.3
        elif user_count <= 2:
            bar_height = 0.4
        ...
        # 固定 Y 轴位置，让条形居中显示
        y_positions = list(range(len(self.current_names)))
        self.ax.barh(y_positions, current_values, color=COLOR_ACCENT_BLUE, height=bar_height, alpha=0.9)
        self.ax.set_yticks(y_positions)
        self.ax.set_yticklabels(self.current_names)
        
        # 固定 Y 轴范围，让少量用户时条形不会太大
        self.ax.set_ylim(-0.5, max(4.5, len(self.current_names) - 0.5))
...
class HistoryFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.toast_label = ctk.CTkLabel(self.toast_frame, text="", font=("Microsoft YaHei UI", 12, "bold"),
                                        text_color="#2E7D32")
    ...
    def request_edit_mode(self):
        ...
        if pwd == ADMIN_PASSWORD:
            ...
            # 【修复3】使用本地 Toast 而非 TimerFrame 的 Toast
    ...
    def delete_bulk(self):
        ...
        try:
            ...
            # 【修复3】使用本地 Toast
            self.show_local_toast(f"✅ 已删除 {len(indices_to_delete)} 条记录", "#E8F5E9", "#2E7D32")
    ...
    def load_data(self, filter_text=""):
        ...
        try:
            ...
            print(f"读取文件失败: {e}")
            ctk.CTkLabel(self.scroll, text="读取失败", text_color="gray").pack(pady=50)
            return
    ...
    def create_row(self, user, start, end, dur, index):
        ...
        if not self.is_edit_mode:
            ctk.CTkButton(right_box, text="×", width=30, height=30, fg_color="#FFE5E5", text_color="#FF3B30",
                          hover_color="#FFD1D1",
                          command=lambda idx=index: self.ask_delete_single(idx)).pack(side="right")

    def ask_delete_single(self, index):
        """【修复4】单条删除，使用闭包捕获 index 并立即刷新"""
